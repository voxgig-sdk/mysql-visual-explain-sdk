...
boru-driven command-line client **and** interactive REPL for the MysqlVisualExplain
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mysql-visual-explain-cli
   ```
...
Run with no arguments to open a REPL (prompt `mysql-visual-explain>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `query_analysi`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mysql-visual-explain-cli`. |
...
(`./mysql-visual-explain-cli <expr>`) and interactively (the REPL), and expressions compose the
