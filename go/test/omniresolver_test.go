...
import (
	...
	omni "github.com/voxgig-sdk/mysql-visual-explain-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MysqlVisualExplainError); is {
				out := map[string]any{
					"name":    "MysqlVisualExplainError",
...
func omniCtx(arg any, client *sdk.MysqlVisualExplainSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MysqlVisualExplainSDK); is {
			client = live
