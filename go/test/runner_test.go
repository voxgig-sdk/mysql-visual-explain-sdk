...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MYSQLVISUALEXPLAIN_TEST_LIVE") == "TRUE" ||
		os.Getenv("MYSQLVISUALEXPLAIN_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MYSQLVISUALEXPLAIN_TEST_EXPLAIN"); explain != "" {
