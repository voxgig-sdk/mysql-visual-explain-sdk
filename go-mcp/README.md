...
[MCP](https://modelcontextprotocol.io) server exposing the MysqlVisualExplain SDK as
two agent tools — `mysql-visual-explain_list` and `mysql-visual-explain_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mysql-visual-explain-mcp)
make build
...
```jsonc
// mysql-visual-explain_list: first page of records
{ "entity": "query_analysi" }
{ "entity": "query_analysi", "query": { } }

// mysql-visual-explain_load: one record by id
{ "entity": "system_info", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mysql-visual-explain-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mysql-visual-explain \
     -- "$PWD"/dist/*/mysql-visual-explain-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mysql-visual-explain_list` and `mysql-visual-explain_load` tools now appear
   in new sessions. Ask the agent to *"list query_analysi using mysql-visual-explain"*
   and it calls `mysql-visual-explain_list` with `{"entity":"query_analysi"}`.
...
### Call the `mysql-visual-explain_list` tool
...
### Call the `mysql-visual-explain_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mysql-visual-explain_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mysql-visual-explain_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MYSQL_VISUAL_EXPLAIN_APIKEY` | API key sent with every request. |
| `MYSQL_VISUAL_EXPLAIN_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mysql-visual-explain_load","arguments":{"entity":"system_info","query":{"id":1}}}}'
```
