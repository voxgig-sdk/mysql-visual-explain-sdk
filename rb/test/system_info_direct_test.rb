...
class SystemInfoDirectTest < Minitest::Test
  def test_direct_load_system_info
    setup = system_info_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-system_info", setup[:live] ? "live" : "unit")
    if _should_skip
...
def system_info_direct_setup(mockres)
  ...
  if live
    ...
    client = MysqlVisualExplainSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
