...
This API provides a visualization tool that helps users understand and optimize their MySQL queries through easy-to-read graphical outputs.

Learn more about MySQL Visual Explain at [mysqlexplain.com](https://mysqlexplain.com).
...
This SDK exposes the API as a small set of **semantic entities** — QueryAnalysi and SystemInfo — that you
...
const queryanalysi = await client.QueryAnalysi().create({
  query: 'example',
})
```
...
// systeminfo is the SystemInfo entity, populated with mock data
...
```go
client := sdk.Test()
result, err := client.SystemInfo(nil).Load(
    nil, nil,
)
```
