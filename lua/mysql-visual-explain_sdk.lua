...
require("mysql-visual-explain_types")
...
local MysqlVisualExplainSDK = {}
MysqlVisualExplainSDK.__index = MysqlVisualExplainSDK
...
MysqlVisualExplainSDK._make_feature = _make_feature


function MysqlVisualExplainSDK.new(options)
  local self = setmetatable({}, MysqlVisualExplainSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MysqlVisualExplainSDK:options_map()
...
function MysqlVisualExplainSDK:get_utility()
  return Utility.copy(self._utility)
end


function MysqlVisualExplainSDK:get_root_ctx()
  return self._rootctx
end


function MysqlVisualExplainSDK:prepare(fetchargs)
...
function MysqlVisualExplainSDK:direct(fetchargs)
...
function MysqlVisualExplainSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MysqlVisualExplainSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MysqlVisualExplainSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MysqlVisualExplainSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MysqlVisualExplainSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MysqlVisualExplainSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MysqlVisualExplainSDK:QueryAnalysi(data)
  ...
  if data == nil then
    if self._query_analysi == nil then
      self._query_analysi = EntityMod.new(self, nil)
    end
    return self._query_analysi
  end
...
function MysqlVisualExplainSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MysqlVisualExplainSDK.new(sdkopts)
  sdk.mode = "test"
