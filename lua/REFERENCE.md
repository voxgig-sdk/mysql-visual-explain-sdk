# MysqlVisualExplain Lua SDK Reference

Complete API reference for the MysqlVisualExplain Lua SDK.
...
```lua
local result, err = client:QueryAnalysi(nil):create({
  query = --[[ `$STRING` ]],
}, nil)
```
...
```lua
local result, err = client:SystemInfo(nil):load({ id = "system_info_id" }, nil)
```
