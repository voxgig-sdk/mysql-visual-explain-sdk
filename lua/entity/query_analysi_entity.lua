...
local QueryAnalysiEntity = {}
QueryAnalysiEntity.__index = QueryAnalysiEntity


function QueryAnalysiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QueryAnalysiEntity)
  self._name = "query_analysi"
  self._client = client
...
function QueryAnalysiEntity:get_name()
  return self._name
end


function QueryAnalysiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QueryAnalysiEntity.new(self._client, opts)
end
...
function QueryAnalysiEntity:mark_deleted()
  self._deleted = true
end


function QueryAnalysiEntity:deleted()
  return true == self._deleted
end


function QueryAnalysiEntity:data_set(args)
  if args ~= nil then
...
function QueryAnalysiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QueryAnalysiEntity:match_set(args)
  if args ~= nil then
...
function QueryAnalysiEntity:match_get()
...
function QueryAnalysiEntity:stream(action, args, callopts)
...
function QueryAnalysiEntity:_run_op(ctx, post_done)
  local utility = self._utility
