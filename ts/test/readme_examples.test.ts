...
//     (\`new MysqlVisualExplainSDK(...)\` / \`MysqlVisualExplainSDK.test(...)\` become
//     \`MysqlVisualExplainSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MysqlVisualExplainSDK'
...
const TEST_SEED = {"entity":{"query_analysi":{"test01":{"id":"test01"}},"system_info":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MysqlVisualExplainSDK(...)\` and any
// \`MysqlVisualExplainSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MysqlVisualExplainSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MysqlVisualExplainSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
