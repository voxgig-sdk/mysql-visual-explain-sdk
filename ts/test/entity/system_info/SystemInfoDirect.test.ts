...
describe('SystemInfoDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new MysqlVisualExplainSDK({
  ...
  test('direct-load-system_info', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-system_info', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MYSQL_VISUAL_EXPLAIN_TEST_SYSTEM_INFO_ENTID']
