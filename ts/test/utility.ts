...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MYSQLVISUALEXPLAIN_TEST_LIVE ||
    'TRUE' === process.env.MYSQLVISUALEXPLAIN_TEST_OVERRIDE
  ) {
  ...
  m.MYSQLVISUALEXPLAIN_TEST_EXPLAIN = process.env.MYSQLVISUALEXPLAIN_TEST_EXPLAIN || m.MYSQLVISUALEXPLAIN_TEST_EXPLAIN
