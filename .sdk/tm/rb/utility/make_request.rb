...
module MysqlVisualExplainUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MysqlVisualExplainResponse.new({})
    ...
    if fetch_err
      ...
      response = MysqlVisualExplainResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MysqlVisualExplainResponse.new(fetched)
    else
