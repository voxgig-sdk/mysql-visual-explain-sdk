...
// MysqlVisualExplain SDK retry feature
...
class MysqlVisualExplainRetryFeature extends MysqlVisualExplainBaseFeature
{
    ...
    public function init(MysqlVisualExplainContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MysqlVisualExplainContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MysqlVisualExplainContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MysqlVisualExplainContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
