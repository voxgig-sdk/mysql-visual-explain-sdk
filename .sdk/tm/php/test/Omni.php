...
//    only - a typed MysqlVisualExplainContext there would make every ctx
...
final class MysqlVisualExplainOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MysqlVisualExplainContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MysqlVisualExplainSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MysqlVisualExplainResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MysqlVisualExplainError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MysqlVisualExplainResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MysqlVisualExplainError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MysqlVisualExplainError(is_string($code) ? $code : '', $msg);
    }
