...
// MysqlVisualExplain SDK feature test
//
...
class FtCtrl extends MysqlVisualExplainControl
{
...
class FtClient
{
    ...
    private MysqlVisualExplainUtility $utility;

    public function __construct(MysqlVisualExplainUtility $utility, array $options)
    {
    ...
    public function get_utility(): MysqlVisualExplainUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MysqlVisualExplainContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MysqlVisualExplainContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MysqlVisualExplainContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MysqlVisualExplainConfig::make_feature($name);
    ...
    private static function buildUrl(MysqlVisualExplainSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MysqlVisualExplainError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MysqlVisualExplainResponse($fetched);
        }
    ...
    private function fail(MysqlVisualExplainContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MysqlVisualExplainContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
