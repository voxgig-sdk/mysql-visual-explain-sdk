# MysqlVisualExplain SDK test runner
...
class MysqlVisualExplainTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MysqlVisualExplainTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MysqlVisualExplainTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MysqlVisualExplainTestRunner.getenv("MYSQL_VISUAL_EXPLAIN_TEST_LIVE")
        override = MysqlVisualExplainTestRunner.getenv("MYSQL_VISUAL_EXPLAIN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MysqlVisualExplainTestRunner.getenv(key)
        ...
        explain = MysqlVisualExplainTestRunner.getenv("MYSQL_VISUAL_EXPLAIN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MysqlVisualExplainTestRunner._test_control is not None:
            return MysqlVisualExplainTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MysqlVisualExplainTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MysqlVisualExplainTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MysqlVisualExplainTestRunner.load_test_control()
...
def load_env_local():
    MysqlVisualExplainTestRunner.load_env_local()


def env_override(m):
    return MysqlVisualExplainTestRunner.env_override(m)


def entity_data(v):
    return MysqlVisualExplainTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MysqlVisualExplainTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MysqlVisualExplainTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MysqlVisualExplainTestRunner.load_test_control()


def live_client_options():
    return MysqlVisualExplainTestRunner.live_client_options()


def live_delay_ms():
    return MysqlVisualExplainTestRunner.live_delay_ms()
