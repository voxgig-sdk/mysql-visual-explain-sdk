...
class TestSystemInfoDirect:

    def test_should_direct_load_system_info(self):
        setup = _system_info_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-system_info", "live" if setup["live"] else "unit")
        if _skip:
...
def _system_info_direct_setup(mockres):
